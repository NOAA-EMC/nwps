...
cat /dev/null > ${grib2dir}/tracking/CG0/nwps.t${hh}z.cg0_trkng.${siteid}.grib2

echo "Creating final grib2 file: ${grib2dir}/tracking/CG0/nwps.t${hh}z.cg0_trkng.${siteid}.grib2"
for TYPE in HSIGN DIR TP
do
    cat ${WTVARdir}/${TYPE}_final.grib2 >> ${grib2dir}/tracking/CG0/nwps.t${hh}z.cg0_trkng.${siteid}.grib2
done
...
if [ "${SENDCOM}" == "YES" ]; then
   mkdir -p $COMOUTCYC
   cp -fv  ${grib2dir}/tracking/CG0/nwps.t${hh}z.cg0_trkng.${siteid}.grib2 $COMOUTCYC/
   if [ "${SENDDBN}" == "YES" ]; then
       ${DBNROOT}/bin/dbn_alert MODEL NWPS_GRIB $job  ${COMOUTCYC}/nwps.t${hh}z.cg0_trkng.${siteid}.grib2
   fi
fi
