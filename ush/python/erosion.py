...
for tstep in range(1, (TDEF+1)):
   ...
   if tstep == 1:
      command = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 master_table=2 parmcat=3 parm=3:surface:anl" | $WGRIB2 -i '+DSET+' -rpn "sto_1:-9999:rcl_1:merge" -spread '+grib2dump
      command2 = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 master_table=2 parmcat=3 parm=3:surface:anl" | $WGRIB2 -i '+DSET+' -max | cat > '+fieldmax
      command3 = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 master_table=2 parmcat=3 parm=3:surface:anl" | $WGRIB2 -i '+DSET+' -min | cat > '+fieldmin
   else:
      command = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 master_table=2 parmcat=3 parm=3:surface:'+str((tstep-1)*TINCR)+' hour" | $WGRIB2 -i '+DSET+' -rpn "sto_1:-9999:rcl_1:merge" -spread '+grib2dump
      command2 = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 master_table=2 parmcat=3 parm=3:surface:'+str((tstep-1)*TINCR)+' hour" | $WGRIB2 -i '+DSET+' -max | cat >> '+fieldmax
      command3 = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 master_table=2 parmcat=3 parm=3:surface:'+str((tstep-1)*TINCR)+' hour" | $WGRIB2 -i '+DSET+' -min | cat >> '+fieldmin
   os.system(command)
   os.system(command2)
   os.system(command3)
...
for tstep in range(1, (TDEF+1)):
   ...
   figtitle = '** EXPERIMENTAL **   NWPS-USGS Dune Erosion Probability (%) \n Hour '\
