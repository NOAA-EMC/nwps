...
for tstep in range(1, (TDEF+1)):
   ...
   if tstep == 1:
      ...
      command = '$WGRIB2 '+DSET+' -s | grep "parm=193:surface:anl" | $WGRIB2 -i '+DSET+' -rpn "sto_1:-9999:rcl_1:merge" -spread '+grib2dump     #Note: To be replaced with above line once "parameterName" is updated in $WGRIB2
   else:
      #command = '$WGRIB2 '+DSET+' -s | grep "WLENG:surface:'+str((tstep-1)*TINCR)+' hour" | $WGRIB2 -i '+DSET+' -spread '+grib2dump
      command = '$WGRIB2 '+DSET+' -s | grep "parm=193:surface:'+str((tstep-1)*TINCR)+' hour" | $WGRIB2 -i '+DSET+' -rpn "sto_1:-9999:rcl_1:merge" -spread '+grib2dump     #Note: To be replaced with above line once "parameterName" is updated in $WGRIB2
   os.system(command)
