...
for tstep in range(1, (TDEF+1)):
   ...
   if tstep == 1:
      command = 'wgrib2 '+DSET+' -s | grep "HTSGW:surface:anl" | wgrib2 -i '+DSET+' -spread '+grib2dump
   else:
      command = 'wgrib2 '+DSET+' -s | grep "HTSGW:surface:'+str((tstep-1)*TINCR)+' hour" | wgrib2 -i '+DSET+' -spread '+grib2dump
   os.system(command)
