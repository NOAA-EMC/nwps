...
for tstep in range(TSTART, (int(TEND)+1)):
   ...
   if tstep == 1:
      command = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 center=7 local_table=1 parmcat=3 parm=253:surface:anl" | $WGRIB2 -i '+DSET+' -rpn "sto_1:-9999:rcl_1:merge" -spread '+grib2dump
      #command2 = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 center=7 local_table=1 parmcat=3 parm=253:surface:anl" | $WGRIB2 -i '+DSET+' -max | cat > '+fieldmax
      #command3 = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 center=7 local_table=1 parmcat=3 parm=253:surface:anl" | $WGRIB2 -i '+DSET+' -min | cat > '+fieldmin
   else:
      command = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 center=7 local_table=1 parmcat=3 parm=253:surface:'+str((tstep-1)*TINCR)+' hour" | $WGRIB2 -i '+DSET+' -rpn "sto_1:-9999:rcl_1:merge" -spread '+grib2dump
      #command2 = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 center=7 local_table=1 parmcat=3 parm=253:surface:'+str((tstep-1)*TINCR)+' hour" | $WGRIB2 -i '+DSET+' -max | cat >> '+fieldmax
      #command3 = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 center=7 local_table=1 parmcat=3 parm=253:surface:'+str((tstep-1)*TINCR)+' hour" | $WGRIB2 -i '+DSET+' -min | cat >> '+fieldmin
   os.system(command)
   #os.system(command2)
   #os.system(command3)
...
fieldmax = 'OWASH_extract_fieldmax_TSTART'+str(TSTART)+'.txt'
command = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 center=7 local_table=1 parmcat=3 parm=253" | $WGRIB2 -i '+DSET+' -max | cat > '+fieldmax
os.system(command)
fieldmin = 'OWASH_extract_fieldmin_TSTART'+str(TSTART)+'.txt'
command = '$WGRIB2 '+DSET+' -s | grep "var discipline=10 center=7 local_table=1 parmcat=3 parm=253" | $WGRIB2 -i '+DSET+' -min | cat > '+fieldmin
os.system(command)
...
for tstep in range(TSTART, (int(TEND)+1)):
   ...
   figtitle = 'NWPS-USGS Overwash Probability (%) \n Hour '\
...
for tstep in range(TSTART, (int(TEND)+1)):
   os.system('rm OWASH_extract_f'+str((tstep-1)*TINCR).zfill(3)+'.txt')
