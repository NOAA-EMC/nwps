...
for tstep in range(TSTART, (int(TEND)+1)):
   ...
   if tstep == 1:
      command = '$WGRIB2 '+DSET+' -s | grep "OWASHP:surface:anl" | $WGRIB2 -i '+DSET+' -rpn "sto_1:-9999:rcl_1:merge" -spread '+grib2dump
   else:
      command = '$WGRIB2 '+DSET+' -s | grep "OWASHP:surface:'+str((tstep-1)*TINCR)+' hour" | $WGRIB2 -i '+DSET+' -rpn "sto_1:-9999:rcl_1:merge" -spread '+grib2dump
   os.system(command)
...
command = '$WGRIB2 '+DSET+' -s | grep "OWASHP" | $WGRIB2 -i '+DSET+' -max | cat > '+fieldmax
os.system(command)
fieldmin = 'OWASH_extract_fieldmin_TSTART'+str(TSTART)+'.txt'
command = '$WGRIB2 '+DSET+' -s | grep "OWASHP" | $WGRIB2 -i '+DSET+' -min | cat > '+fieldmin
os.system(command)
