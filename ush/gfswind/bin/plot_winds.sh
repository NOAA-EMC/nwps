...
# -----------------------------------------------------------
#
# Script used to plot GFS winds input ASCII point files
#
# -----------------------------------------------------------
...
TEMPDIR=${VARdir}/${SITEID}.tmp/gfswind
mkdir -p ${TEMPDIR}
echo "Writing all temp files to ${TEMPDIR}"

GRAPHICSdir="${GRADSOUTPUTdir}/${SITEID}/gfswind"
mkdir -p ${GRAPHICSdir} 
...
then
    ...
    echo "ERROR - Can't find ${INPUTdir}/gfswind_domain.txt"
    export err=1; err_chk
fi
...
then
    ...
    echo "ERROR - Can't find ${INPUTdir}/gfswind_start_time.txt"
    export err=1; err_chk
fi

epoc_time=$(cat ${INPUTdir}/gfswind_start_time.txt)

DOMAIN=$(cat ${INPUTdir}/gfswind_domain.txt | awk -F: '{ print $2 }')
...
echo "GFSWINDDOMAIN = ${DOMAIN}"
echo "GFSWINDNX = ${NX}"
echo "GFSWINDNY = ${NY}"

# Set our script variables from the global config
echo "Plotting GFS wind images"

cd ${INPUTdir}
files=`ls --color=none -1rat gfswind*.dat`
...
tar xvfz ${SHAPEFILEdb}/lpoly_hires.tar.gz
...
echo "Creating GRADS plots for GFS winds as PNG images"
for file in $files
do
    echo "Processing ${file}"
    ${WRITEDAT} ${INPUTdir}/${file} ${TEMPDIR}/wind.bin
    FHOUR=$(echo "${file}" | sed s/.dat//g | awk -F_ '{ print $5 }' | sed s/f//g)
    ...
    ${GRADS} -b -l -c wind.gs
    mv wind.png ${GRAPHICSdir}/gfswind_f${FHOUR}.png
done
