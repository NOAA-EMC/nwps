...
#THIS PART IS READY TO ACTIVATE THE WAVE PARTITION IN SWAN, NEEDS TO BE TESTED
...
if [ "${WVTRCK}" = "ON" ] 
  ...
  if [ "${WVTONCG}" == "0" ]
    ...
    distx=`perl ${WD}/dist_lat_lon.pl $SWLATWT $SWLONWT $SWLATWT $NELONWT`
    disty=`perl ${WD}/dist_lat_lon.pl $SWLATWT $SWLONWT $NELATWT $SWLONWT`
...
distx=`perl ${WD}/dist_lat_lon.pl $SWLAT $SWLON $SWLAT $NELON`
disty=`perl ${WD}/dist_lat_lon.pl $SWLAT $SWLON $NELAT $SWLON`
...
##echo "BREAK POINT 1: NESTGRIDS=${NESTGRIDS} NESTCG=${NESTCG}"

if [ ${NESTGRIDS} -gt 0 ] && [ ${NESTCG} == "Yes" ]
then
    for (( j = 1 ; j < ${NESTGRIDS}+1 ; j++ ))
    do
	distxN[$j]=`perl ${WD}/dist_lat_lon.pl ${SWLATN[$j]} ${SWLONN[$j]} ${SWLATN[$j]} ${NELONN[$j]}`
	distyN[$j]=`perl ${WD}/dist_lat_lon.pl ${SWLATN[$j]} ${SWLONN[$j]} ${NELATN[$j]} ${SWLONN[$j]}`
