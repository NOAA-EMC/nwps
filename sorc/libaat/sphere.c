 ...
 *
 * ARGUMENTS
 * lon1 = longitude of 1st point (Input)
 * lat1 = latitude of 1st point (Input)
 * lon2 = longitude of 2nd point (Input)
 * lat2 = latitude of 2nd point (Input)
 *  ans = latitude where the segment crosses the dateline. (Output)
 *
 * RETURNS: int
 * 0 = OK
 * 1 = longitudes are ill chosen.
 *
 * HISTORY
 * 12/2007 AAT: Added to libaat from mymapf.c in degrib source.
 *
 * NOTES
 *****************************************************************************
 */
                ...
                double *ans)
{
   ...
   /* Get lon1, lon2 in range of 0..360 */
   while (lon1 < 0) {
      lon1 += 360;
   }
   while (lon1 >= 360) {
   ...
   /* Check if 180 is between lon1 and lon2. */
   *ans = lat1 + (lat2 - lat1) * (180 - lon1) / (lon2 - lon1);
