...
!
      integer :: i
      integer :: ier,nobs,nhs,timestep,nshore,istart,loc,rf2use
      integer, dimension(ninput) :: rf2use2
...
!        get the beach slope and slope variability at each transect from USGS slope file
      call read_slope(fuslope,ninput,istart,xpos,ypos,sxpos,sypos,
     1                rf2use2,slope,slope95,slope05,dhigh,dlow,nshore,
     2                ier,esi)
...
!      open(UNIT=furun,FILE='tmp.txt',ACCESS='APPEND')

      call runupcalc(nhs,hsig,pwp,slo,slo95,slo05,rf2use,watlev,dl,es,
     1               setup,sinc,sig,s,runup,runup95,runup05)
!nhs,hs,pwp,slope,slope95,slope05,rf2use,
...
!     cap all output variables if runup too large; set to flag value 999   
      do i=1,nhs
         if (runup(i).eq.999) then
            twl(i)=999
            twl95(i)=999
            twl05(i)=999
            owash(i)=999
...
!        write the output file
     ...
     3       dh,dl,owash(i),erosn(i),owashexd(i),erosnexd(i),rf2use
 100   format(F14.4,4X,F8.4,5X,F8.4,4X,F5.3,5X,F7.4,5X,F6.4,4X,F6.2,7X,
     1       F6.2,5X,F6.2,10X,F6.2,10X,F6.2,10X,F6.2,11X,F5.2,10X,
     2       F4.2,12X,F4.2,17X,F4.2,11X,F8.4,10X,F8.4,
     3       7X,F8.2,9X,F6.2,12X,I3,15X,I3,15X,I1)
      end do
      close(furun)
!
      stop
