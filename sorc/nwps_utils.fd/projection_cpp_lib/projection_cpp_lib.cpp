...
void Projection::AWIPSDomainToLatLon(AWIPSdomain &domain)
{
  ...
  // 04/25/2015: Fix for southern hemisphere when domain spans about and below the equiator
  // if(FloatLessThan(nelat_temp, 0)) nelat_temp -= nelat_temp*2;
  if(FloatLessThan(nelat_temp, 0)) {
    nelat_temp = nelat_temp * -1 ;
    if(FloatLessThan(swlat_temp, 0)) swlat_temp = swlat_temp * -1;
  }
  // 04/25/2015: Fix for southern hemisphere when domain spans about and below the equiator
