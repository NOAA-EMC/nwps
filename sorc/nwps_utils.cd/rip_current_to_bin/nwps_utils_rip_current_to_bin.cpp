...
Program used to convert SWAN RIP current output to a Fortran
...
const char *version_string = "4.06";
const char *program_name = "nwps_utils_rip_current_to_bin";
const char *program_description = "Program used convert SWAN RIP current output to a Fortran BIN file";
...
gxString kml_template_file = "rip_points_template.kml";
...
// Default fill value for testing only
...
struct RIP_t {
  RIP_t(int p = -1, float d = 0.0) {
    file_pos = p;
    data = d;
  }
  RIP_t(const RIP_t &ob) {
    Copy(ob);
  }
  RIP_t &operator=(const RIP_t &ob) {
    if(&ob != this) Copy(ob);
    return *this;
  }
  void Copy(const RIP_t &ob) {
...
int GetClusterPointsFilePostion(float Xp, float Yp, GridCoords &grid_coords, 
				RIP_t *cluster_points, float data);
...
int main(int argc, char **argv)
{
  ...
  if(argc < 5) {
    std::cout << "ERROR - Need SWAN_RIP_OUTPUT_FILE rip_current.meta templates.grib2 points.bin" << "\n";
    HelpMessage();
    return 1;
  }
  ...
  while(narg < argc) {
	...
	if(argc_count == 1) meta_fname = arg;
	if(argc_count == 2) tfname = arg;
	if(argc_count == 3) ofname = arg;
	argc_count++;
      }
    }
    arg = argv[++narg];
  ...
  gxListNode<RIP_points *> *points_ptr, *points_ptr_next;
  int num_time_steps = 0;

  if(SetGridCoords(meta_fname.c_str(), grid_coords) != 0) return 1;

  std::cout << "Opening SWAN RIP current file " << fname.c_str() << "\n";
  ifile.df_Open(fname.c_str());
  if(!ifile) {
    std::cout << "ERROR - Cannot open SWAN RIP current file " 
	      << fname.c_str() << "\n";
  ...
  gxList<RIP_points *> rip_points_list;
  gxListNode<RIP_t> *grid_points;
  ...
  while(ptr) {
    ...
    if(num_arr < 4) {
      std::cout << "ERROR - Missing RIP values in SWAN RIP current file" << "\n" << std::flush;
    ...
    RIP_t cluster_points[num_cluster_points];
    ...
    if(GetClusterPointsFilePostion(Xp, Yp, grid_coords, cluster_points, Prob) != 0) {
      std::cout << "ERROR - Bad cluster center point, Prob LAT/LON grid point is outside of CG grid" << "\n";
      error_level = 1;
      break;
    }

    points_ptr = rip_points_list.GetHead();
    while(points_ptr) {
      if(points_ptr->data->DATE == DATE) break;
      points_ptr = points_ptr->next;
    }

    RIP_t rip_t((int)file_pos, Prob);
    if(!points_ptr) {
      RIP_points *rip_point = new RIP_points; 
      rip_point->DATE = DATE;

      if(use_cluster_points) {
	for(int ncp = 0; ncp < num_cluster_points; ncp++) {
	  rip_point->points.Add(cluster_points[ncp]);
	}
      }
      else {
	rip_point->points.Add(rip_t);
      }
      rip_points_list.Add(rip_point);
      num_time_steps++;
    }
    else {
      if(use_cluster_points) {
	...
	points_ptr->data->points.Add(rip_t);
      }
    }
  ...
  while(points_ptr) {
    ...
    while(grid_points) {
      // latlon_grid[grid_points->data.file_pos].data = grid_points->data.data;

      int percent_prob = roundf(grid_points->data.data);
      if(percent_prob > 100) percent_prob = 100;
      if(percent_prob < 0) percent_prob = 0;
      latlon_grid[grid_points->data.file_pos].data = (float)percent_prob; 
	...
	std::cout << "Prob points = " << year.c_str() << month.c_str() << day.c_str() 
  ...
  if(error_level != 0) { 
    ...
    std::cout << "INFO: ${WGRIB2} " << tfname.c_str() << " -no_header -import_bin " << ofname.c_str() << " -grib_out final_rip.grib2" << "\n";
  }
...
int SetGridCoords(char *fname, GridCoords &grid_coords)
{
  ...
  if(gen_kml_template) {
    ...
    kml_string << "  <Folder>\n";
    kml_string << "    <name>SITEID CGNUM RIP Points</name>\n";
    kml_string << "    <description>SITEID CGNUM RIP Points</description>\n";
...
int ProcessArgs(int argc, char *argv[])
{
  ...
  for(i = 1; i < argc; i++ ) {
	...
	case 'k':
	  ...
	  if(kml_template_file.is_null()) kml_template_file = "rip_points_template.kml";
	  break;
...
void HelpMessage()
{
    program_version();
    std::cout << "Usage:" << "\n";
    std::cout << "          " << process_name << " SWAN_RIP_OUTPUT_FILE rip_current.meta templates.grib2 points.bin" << "\n";
    std::cout << "args:" << "\n";
    ...
    std::cout << "       -k\"template.kml\"    Create a KML template to verify RIP points" << "\n";
    std::cout << "\n";
    std::cout << "Example:" << "\n";
    std::cout << "           " << process_name << " mfl_nwps_5m_CG2_ripprob.20161110_0600 rip_current.meta templates.grib2 points.bin" << "\n";
    std::cout << "\n";
    std::cout << "Usage1: - Enable verbose, debug, and debug levels" << "\n";
    std::cout << "         " << process_name << " -v -d SWAN_RIP_OUTPUT_FILE  rip_current.meta templates.grib2 points.bin" << "\n";
    std::cout << "         " << process_name << " -v -D5 SWAN_RIP_OUTPUT_FILE  rip_current.meta templates.grib2 points.bin" << "\n";
    std::cout << "\n";
    std::cout << "\n";
}
// ----------------------------------------------------------- //
