...
int LoadOrBuildMetaFile(char *fname, GRIB2Message *g2message, int debug)
{
  ...
  // 04/24/2015: Fix for domains below the equator
  ...
  if(!str) {
    ...
    // 04/29/2015: Fix for domains that span the equator
    if(sbuf[0] == '-') {
      lat_is_neg = 1;
    }
    ...
    // 04/29/2015: Fix for domains below the equator
    if(lat_is_neg == 1) {
      ival += setmsb;
    }
