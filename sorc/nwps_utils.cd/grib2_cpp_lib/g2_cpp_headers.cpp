...
unsigned char *g2Section7::GetFileBuf(int withdata)
{
  size_t len = GetSize(withdata);
  unsigned char *buf = new unsigned char[len];
  memset(buf, 0, len);
  g2_reverse_byte_order(id.section_length, 4);
...
int g2Section7::AllocData(size_t len, int withdata)
{
...
size_t g2Section7::GetSize(int withdata)
{
  // 04/11/2016: Fix for wgrib2 # define CHECK
  if(withdata == 0) {
    int seclen = sizeof(id);
    memmove(id.section_length, &seclen, 4);
    return sizeof(id);
  }
  else {
    size_t sz = sizeof(id);
...
size_t g2Section7::SetSize(int withdata)
{
  return GetSize(withdata);
}
