...
{
   if (map->f_latlon == 1) {
      *lon = map->lon1 + (x - 1) * (map->lonN - map->lon1) / (map->Nx - 1.);
      while (*lon > 180)
         *lon = *lon - 360;
      while (*lon < -180)
         *lon = *lon + 360;

      *lat = map->lat1 + (y - 1) * (map->latN - map->lat1) / (map->Ny - 1.);
      if (*lat > 90)
         *lat = 90;
      if (*lat < -90)
         *lat = -90;
   } else {
      /* Reason for 1 + (y - 1) * ratio is that the 1, 1 location is fixed and
       * correct.  The 1, 2 location is off by 1 (not 2) ratio amounts. */
      y = 1 + (y - 1) * map->ratio;
 ...
 *
 * NOTES
 *   Can't use "const myMaparam * map because cll2xy ignores it.
 *****************************************************************************
 */
void myCll2xy (myMaparam *map, double lat, double lon, double *x, double *y)
{
   if (map->f_latlon == 1) {
      ...
      /* Get lon in range of [(lonN-360)..lonN] */
      while (lon < map->lonN - 360)
         lon += 360;
      while (lon > map->lonN)
         lon -= 360;
      *x = 1 + (lon - map->lon1) * ((map->Nx - 1) / (map->lonN - map->lon1));
      *y = 1 + (lat - map->lat1) * ((map->Ny - 1) / (map->latN - map->lat1));
   } else {
 ...
 *   3/2005 AAT: Adjusted to handle lat1 > lat2 regardless of scan mode.
 *
 * NOTES
 *****************************************************************************
 */
...
{
   double orient;       /* Orientation longitude of projection (where N is up.)
                         * (between -180 and 180) */
   ...
   if (gds->projType == GS3_LATLON) {
      ...
      /* Get lon1 and lonN in range of [0..360) */
      while (map->lon1 < 0) {
         map->lon1 += 360;
      }
      while (map->lon1 >= 360) {
      ...
      /* If lonN < lon1, then set lon1 to lon1 - 360. */
      /* Result is that lonN > lon1, lonN - lon1 < 360. */
      /* lonN in [0,360), lon1 in (-360,360) */
      if (map->lonN <= map->lon1) {
         map->lon1 -= 360;
      }

      if (gds->lat1 < gds->lat2) {
         map->lat1 = gds->lat1;
         map->latN = gds->lat2;
      } else if (gds->lat1 > gds->lat2) {
         map->lat1 = gds->lat2;
         map->latN = gds->lat1;
      } else {
         /* Should not get here */
       ...
       * grid.  The branch cut is 180 degree away from the arbitrary orientLon */
                    ...
                    int *y2, gdsType *gds, gdsType *newGds)
{
   ...
   *y2 = (int) ceil (y);
   ...
   myAssert ((gds->projType == GS3_LATLON) || (gds->projType == GS3_POLAR) ||
             (gds->projType == GS3_LAMBERT) || (gds->projType == GS3_MERCATOR));
