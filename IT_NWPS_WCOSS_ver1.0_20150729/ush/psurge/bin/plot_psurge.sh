...
# -----------------------------------------------------------
#
# Script used to plot PSURGE input ASCII point files
#
# -----------------------------------------------------------
if [ "${USHnwps}" == "" ]
    then 
...
if [ ! -e ${INPUTdir}/psurge_waterlevel_domain.txt ]
then
    echo "ERROR - Missing domain file in input DIR"
    echo "ERROR - Can't find ${INPUTdir}/psurge_waterlevel_domain.txt"
    export err=1; err_chk
fi
...
then
    ...
    echo "ERROR - Can't find ${INPUTdir}/psurge_waterlevel_start_time.txt"
    export err=1; err_chk
fi

epoc_time=$(cat ${INPUTdir}/psurge_waterlevel_start_time.txt)

DOMAIN=$(cat ${INPUTdir}/psurge_waterlevel_domain.txt | awk -F: '{ print $2 }')
...
do
    ...
    mv waterlevel.png ${GRAPHICSdir}/psurge_waterlevel_f${FHOUR}.png
done
